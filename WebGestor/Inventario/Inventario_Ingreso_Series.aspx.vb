...
Partial Class Inventario_Inventario_Ingreso_Series
    ...
    Protected Sub btnListar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnListar.Click
        Try
            lblError.Text = ""
            If cboBusAlmacen.SelectedValue = "< Seleccionar >" Then lblError.Text = "<br> - Falta seleccionar el Almacén." : Exit Sub
            Dim pdCodalmacen As Double = cboBusAlmacen.SelectedValue.Trim
            Dim pdCodRecep As String = ""
            Dim psEstado As String = "1,3"
            Dim psFecha As String = ""
            Dim psFechaFin As String = ""
            psFecha = Mid(txtBusFecIni.Text, 7, 4) + Mid(txtBusFecIni.Text, 4, 2) + Mid(txtBusFecIni.Text, 1, 2)
            If txtBusFecFin.Text = "" Then
                psFechaFin = psFecha
            Else
                psFechaFin = Mid(txtBusFecFin.Text, 7, 4) + Mid(txtBusFecFin.Text, 4, 2) + Mid(txtBusFecFin.Text, 1, 2)
            End If

            Dim dt As DataTable
            dt = obj.Lista_Recepcion(Session("Ruta_Emp"), Session("CodEmpresa"), pdCodalmacen, pdCodRecep, psEstado, 0, psFecha, psFechaFin, "", "")
            Flex.DataSource = dt
            Flex.DataBind()
    ...
    Protected Sub Flex_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles Flex.RowCommand
        ...
        If e.CommandName = "Ingreso" Then
            ...
            txtIngCodAlmacen.Text = cboBusAlmacen.SelectedValue.Trim
            txtIngRecepcion.Text = Llenar_Ceros(Flex.Rows(Index).Cells(2).Text, 6)
            txtIngAlmacen.Text = cboBusAlmacen.Items(cboBusAlmacen.SelectedIndex).Text
            txtIngProveedor.Text = Flex.Rows(Index).Cells(9).Text
            Dim dt As New DataTable
            Dim pdCodRecep As Double = txtIngRecepcion.Text
            dt = obj.Lista_Recepcion_Item(Session("Ruta_Emp"), Session("CodEmpresa"), pdCodRecep, "S")
            FlexItemSerie.DataSource = dt
            FlexItemSerie.DataBind()
            dt = Nothing
            dt = obj.Lista_Recepcion_Item(Session("Ruta_Emp"), Session("CodEmpresa"), pdCodRecep, "N")
    ...
    Protected Sub btnRegresar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRegresar.Click
        ...
        FlexItemAcc.Visible = False
        IngSeries.Visible = False
    ...
    Protected Sub FlexItemSerie_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles FlexItemSerie.RowCommand
        ...
        If e.CommandName = "IngSerie" Then
            ...
            txtIngArticulo.Text = FlexItemSerie.Rows(Index).Cells(3).Text
    ...
    Private Sub Llenar_NroSeries()
        ...
        Try
            ...
            For i = 0 To FlexSeries.Rows.Count - 1
                ...
                Dim txtNroPlaca As TextBox = FlexSeries.Rows(i).Cells(1).FindControl("txtPlaca")
    ...
    Private Sub Calculo_Cantidades_Recibidas()
        ...
        CmdGlobal.CommandText = "SELECT ARTICULO_CODIGO," _
             ...
             & " WHERE (SERIE_SYS_EST = '0') AND (NOT(A.ART_TIPO IN ('87','88')))  GROUP BY RECEP_CODIGO, ARTICULO_CODIGO HAVING (RECEP_CODIGO =" & txtIngRecepcion.Text.Trim & ")"
        ...
        If Rs.HasRows Then
            While Rs.Read
                ...
                End If
                Rs2.Close()
                CmdGlobal2.CommandText = " UPDATE TBINV_ALMACEN_RECEPCION_DET SET RECEPD_CANT_REC=" & Nz(Rs!CREC) & ",RECEPD_CANT_FALT_REC=" & Nz(Rs!CFREC) & ",RECEPD_CANT_SOBR=" & Nz(Rs!CSOB) & ",RECEPD_CANT_RECPARCIAL=" & Nz(Crecant) & ",RECEPD_CANT_ING=" & Nz(Rs!CING) & _
        ...
        If Rs.HasRows Then
            While Rs.Read
                CmdGlobal2.CommandText = "UPDATE TBINV_ALMACEN_RECEPCION SET RECEP_CANT_REC=" & Nz(Rs!CREC) & ",RECEP_CANT_FALT_REC=" & Nz(Rs!CFREC) & ",RECEP_CANT_SOBR=" & Nz(Rs!CSOB) & _
    ...
    Protected Sub btnExportar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportar.Click
        ...
        'NomArch = txtIngRecepcion.Text.Trim & " " & txtIngProveedor.Text.Trim


        'Using excelPackage As New ExcelPackage()
        '    ' Agregar hojas al archivo de Excel
        '    Dim worksheet1 = excelPackage.Workbook.Worksheets.Add(txtIngRecepcion.Text.Trim)
        ...
        '    Response.AddHeader("content-disposition", "attachment; filename=BienesInventariados" & NomArch & ".xlsx")
        '    Response.BinaryWrite(excelPackage.GetAsByteArray())
        '    Response.End()
        'End Using


        'Call Exportar_Excel(dt, QuitaPunto(NomArch))
    End Sub
    Private Sub Exportar_Excel(ByVal dt As DataTable, ByVal NomArchivo As String)
        FlexExportar.DataSource = dt
        FlexExportar.DataBind()
        ...
        form.Controls.Add(FlexExportar)
        pagina.RenderControl(htw)
        Response.Clear()
        Response.Buffer = True
        Response.ContentType = "application/vnd.ms-excel"
        Response.AddHeader("Content-Disposition", "attachment;filename=" & NomArchivo & ".xls")
    ...
    Protected Sub btnEjecutar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEjecutar.Click
        ...
        Dim psFechaRecep As String = ""
        Dim dt As DataTable
        Dim pdCodRecepcion As Double = txtIngRecepcion.Text.Trim
        ...
        If dt.Rows.Count > 0 Then
            For Each dr As DataRow In dt.Rows
                ...
                psFechaRecep = Nu(dr("RECEP_FEC_EMI_DOC"))
            Next
        End If
        dt = Nothing
        ...
        ValorSys = ""
        Dim Guia As String = ""
        If psMotivo = "8" And Nz(psNroDocumento) > 0 Then
        ...
        If psTipoDoc = "1" And psPrefijo = "OC" Then
            CmdGlobal.CommandText = " SELECT * FROM TBINV_ALMACEN_RECEPCION_PARTEGUIA WHERE RECEP_CODIGO =" & pdCodRecepcion
            Rs = CmdGlobal.ExecuteReader
            If Rs.HasRows Then
            Else
                If txtGuiaSerie.Text.Trim <> "" And txtGuiaNro.Text.Trim <> "" Then
                    CmdGlobal2.CommandText = " SELECT MAX(GUIA_CODIGO) FROM TBINV_ALMACEN_RECEPCION_PARTEGUIA "
                    Rs2 = CmdGlobal2.ExecuteReader
                    If Rs2.HasRows > 0 Then
                        While Rs2.Read
                            Guia = Nz(Rs2(0)) + 1
                        End While
                    Else
                    ...
                    End If
                    Rs2.Close()
                    CmdGlobal2.CommandText = " INSERT INTO TBINV_ALMACEN_RECEPCION_PARTEGUIA (EMPRESA_CODIGO,RECEP_CODIGO,GUIA_CODIGO,GUIA_SERIE,GUIA_NUMERO)" _
                                          & " VALUES ('" & Session("CodEmpresa") & "'," & pdCodRecepcion & "," & Guia & ",'" & txtGuiaSerie.Text.Trim & "','" & txtGuiaNro.Text.Trim & "')"
                    CmdGlobal2.ExecuteNonQuery()
                End If
            End If
            Rs.Close()
        ...
        If LblErrort.Text <> "" Then
            LblErrort.Text = LblErrort.Text : Exit Sub
        End If
                ...
                If CDbl(Nz(Rs!RECEPD_CANT_REC) - Nz(Rs!RECEPD_CANT_ING)) > 0 Then
                    ...
                    If CDbl(Nz(Rs!RECEPD_CANT_REC) - Nz(Rs!RECEPD_CANT_ING)) > 0 Then
                        If psMotivo = "8" Then 'COMPRA
                            ...
                            End If
                            Rs2.Close()
                            CmdGlobal2.CommandText = "INSERT INTO TBINV_MOV_ALMACEN_INGXCOMPRA2(EMPRESA_CODIGO,INGXC_NUMERAR,RECEP_CODIGO," _
        ...
        If Existe_Tabla("TBLOGIS_ORDENES_COMPRA", Session("Ruta_Emp")) = True Then
            CmdGlobal.CommandText = " SELECT DISTINCT OC.OCOMPRA_CODIGO , OC.OCOMPRA_NUMERAR, RD.DESTINO_TIPO, RD.DESTINO_CODIGO, A.ART_TIPO, A.ART_CODIGO, OC.OCOMPRA_ORDENTRABAJO, R.RECEP_CANT_XREC, R.RECEP_CANT_REC, R.RECEP_CANT_FALT_REC " _
                  ...
                  & " WHERE (RD.DESTINO_TIPO = '4') AND (OC.OCOMPRA_CODIGO = " & psNroDocumento & ") AND (A.ART_TIPO = 89) AND (A.EMPRESA_CODIGO = '" & Session("CodEmpresa") & "') AND " _
            ...
            If psTipoDocumento = "1" Then psOCompra = Nz(psNroDocumento)
            If psOCompra <> "" Then
                CmdGlobal.CommandText = "UPDATE TBLOGIS_ORDENES_COMPRA SET OCOMPRA_ESTADO='6' " _
                                      & "WHERE EMPRESA_CODIGO='" & Session("CodEmpresa") & "' AND OCOMPRA_ESTADO = '1' AND OCOMPRA_NUMERAR=" & Nz(psNroDocumento)
                CmdGlobal.ExecuteNonQuery()
            End If
        End If
        ...
        Rs = CmdGlobal.ExecuteReader
        If Rs.HasRows Then psestOk = True
        Rs.Close()
    ...
    Private Sub BtnCancelarIng_Click(sender As Object, e As EventArgs) Handles BtnCancelarIng.Click
        txtIngArtCodigo.Text = ""
        txtIngArticulo.Text = ""
        Dim dt As New DataTable
        dt = Nothing
    ...
    Protected Sub btnGuardarExcel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGuardarExcel.Click
    ...
    Private Sub LoadExcelData(ByVal filePath As String)
        ...
        ' Añadir columnas
        'For Each cell As IXLCell In worksheet.Row("LECTURA").Cells()
