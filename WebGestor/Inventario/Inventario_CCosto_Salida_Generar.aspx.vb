...
Partial Class Inventario_Inventario_CCosto_Salida_Generar
    ...
    Private Sub Carga_Motivos()
        ...
        Try
            ...
            cmdSql.CommandText = "SELECT DISTINCT MAINSA_MOTIVO_TRASLADO, (SELECT ELEMEN_VALOR FROM BDGRUPOEMPRESAS.DBO.TBCELEMEN WHERE ELEMEN_TABLA = 'TBOPC217' AND ELEMEN_CODIGO = MAINSA_MOTIVO_TRASLADO) AS MOTIVO_TRASLADO" _
                               & " FROM TBINV_MATRIZ_INGRESOSALIDA WHERE (EMPRESA_CODIGO = '" & Session("CodEmpresa") & "') AND (MAINSA_TIPO_MOVIMIENTO = 'S') AND (MAINSA_UBICACION1 = '2') AND (MAINSA_UBICACION2 = '" & IIf(RBAlmacen.Checked = True, "1", "2") & "') ORDER BY MOTIVO_TRASLADO"
    ...
    Protected Sub _Grabar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles _Grabar.Click
        ...
        If txtFechaDevol.Visible = True Then
            ...
            sFecha_Devol = Right(txtFechaDevol.Text.Trim, 4) + Mid(txtFechaDevol.Text.Trim, 4, 2) + Left(txtFechaDevol.Text.Trim, 2)
            ...
            If sFecha_Devol >= Format(FechaSal, "yyyymmdd") Then
            Else
        ...
        Try
            ...
            CmdGlobal.CommandText = "INSERT INTO TBINV_CCOSTO_SALIDA(EMPRESA_CODIGO,OSAL_CODIGO, OSAL_FECHA,OSAL_HORA, OSAL_USUARIO, OSAL_FECHA_SAL, OSAL_HORA_SAL,OSAL_TIPODESTINO," _
                                   & "ALMACEN_CODIGO_DESTINO,CECOSE_CODIGO_DESTINO, CECOSE_CODIGO_ORIGEN,OSAL_CANT_ENV, OSAL_CANT_REC,OSAL_CANT_FALT_REC,OSAL_ESTADO, OSAL_OBSERVACION, OSAL_SYS_EST, OSAL_MOTIVO_GRAL,OSAL_PERSONA_ENVIA) " _
                                   & "VALUES('" & Session("CodEmpresa") & "'," & CLng(lblCodigo.Text) & ",'" & FechaServer & "','" & HoraServer & "','" & lblUsuario.Text & "','" & Format(Convert.ToDateTime(FechaSal), "yyyyMMdd") & "','" & Left(HoraSal, 2) & Right(HoraSal, 2) & "','" & IIf(RBAlmacen.Checked = True, "1", "2") & "'," _
                                   & DesCodAlmacen & "," & DesCodSeccion & "," & CLng(lblCodOrigen.Text) & "," & TotArt & ",0," & TotArt & ",'" & Estado & "','" & txtObs.Text.Trim & "','" & SysEst & "','" & Motivo & "','" & txtPerEnvia.Text.Trim & "')"
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    If cFuncion.SelectedValue.Trim <> "< Seleccionar >" Then
                        CmdGlobal.CommandText = "INSERT TBINV_CCOSTO_SALIDA_DET( EMPRESA_CODIGO, OSAL_CODIGO, OSALD_ORDEN, SERIE_NUMERAR, ENVIADA_OK,RECIBIDA_OK,OSALD_SYS_EST,OSALD_MOTIVO,OSALD_FUNCION) " _
                                              & " VALUES('" & Session("CodEmpresa") & "'," & CLng(lblCodigo.Text) & "," & Item & "," & .Rows(i).Cells(6).Text & ",'S','N','0','" & Motivo & "','" & cFuncion.SelectedValue.Trim & "')"
                        CmdGlobal.ExecuteNonQuery()
                    Else
                        CmdGlobal.CommandText = "INSERT TBINV_CCOSTO_SALIDA_DET( EMPRESA_CODIGO, OSAL_CODIGO, OSALD_ORDEN, SERIE_NUMERAR, ENVIADA_OK,RECIBIDA_OK,OSALD_SYS_EST,OSALD_MOTIVO) " _
                                              & " VALUES('" & Session("CodEmpresa") & "'," & CLng(lblCodigo.Text) & "," & Item & "," & .Rows(i).Cells(6).Text & ",'S','N','0','" & Motivo & "')"
                        CmdGlobal.ExecuteNonQuery()
                    End If
                    CmdGlobal.CommandText = "UPDATE TBINV_ARTICULOS_SERIES_" & Session("CodEmpresa") & " SET UBICACT_TIPO='0',UBICACT_CODIGO=NULL,UBICACT_SYS='" & ValorSys & "' WHERE SERIE_NUMERAR=" & .Rows(i).Cells(6).Text 'TIPO 0: EN TRANSITO
                    CmdGlobal.ExecuteNonQuery()
                    ...
                    Else
                        ...
                        CmdGlobal2.CommandText = "INSERT INTO TBINV_MOVIMIENTO_GENERAL (EMPRESA_CODIGO, MOV_NRO, MOV_TIPO, TIPO_UBICACT, CODIGO_UBICACT,TIPO_ORIGEN_DESTINO,CODIGO_ORIGEN_DESTINO, " _
                                               ...
                                               & " values('" & Session("CodEmpresa") & "','" & NroMovimiento & "','2','2','" & lblCodOrigen.Text & "','" & IIf(RBAlmacen.Checked = True, "1", "2") & "','" & lblCodDestino.Text & "', " _
            ...
            Select Case Motivo
                Case "1" 'prestamo
                    CmdGlobal2.CommandText = "INSERT INTO TBINV_PRESTAMO(EMPRESA_CODIGO, PRESTA_CODIGO, PRESTA_TIPO_MOVIMIENTO, PRESTA_TIPOORIGEN, OSAL_CODIGO, RECEP_CODIGO,DESP_CODIGO," _
                                          ...
                                          & " NULL," & lblCodOrigen.Text & ",NULL,'" & IIf(RBAlmacen.Checked = True, "1", "2") & "'," & DesCodAlmacen & "," & DesCodSeccion & "," _
                                          & " NULL,'" & Motivo & "') "
                    CmdGlobal2.ExecuteNonQuery()
                    Item = 0
                    With _DetalleEq
                        For i = 0 To .Rows.Count - 1 'estado envio: 0 enviado ,1 recibido; estado prestamo: 0 por prestar, 1 prestado,2 por devolver, 3 devuelto, 4 devolver parcial
                            ...
                            CmdGlobal2.CommandText = "INSERT TBINV_PRESTAMO_DETALLE(EMPRESA_CODIGO,PRESTA_CODIGO, PREDET_CODIGO,SERIE_NUMERAR, PREDET_SYS_REGISTRO, PREDET_ESTADO_ENVIO, PREDET_SYS_ENVIO, PREDET_ESTADO_PRESTAMO, " _
                                                  ...
                                                  & " VALUES('" & Session("CodEmpresa") & "'," & NroPrestamo & "," & Item & "," & .Rows(i).Cells(6).Text & ",'" & ValorSys & "','0','" & ValorSys & "','0'," _
                                                  & " NULL,'" & sFecha_Devol & "',NULL)"
                            CmdGlobal2.ExecuteNonQuery()
                        Next
                    End With
                    With _DetalleAc
                        ...
                        For i = 0 To .Rows.Count - 1 'estado envio 0 enviado ,1 recibido; estado prestamo 0 por prestar, 1 prestado,2 por devolver, 3 devuelto, 4 devuelto parcial
                            If RBAlmacen.Checked = True Then 'almacen
                                CmdGlobal2.CommandText = "UPDATE TBINV_PRESTAMO_DETALLE SET PREDET_ESTADO_PRESTAMO = '2',OSAL_CODIGO_DEVOL =" & lblCodigo.Text & ",DESP_CODIGO_DEVOL = NULL,RECEP_CODIGO_DEVOL = NULL FROM TBINV_PRESTAMO A, TBINV_PRESTAMO_DETALLE B WHERE A.EMPRESA_CODIGO = B.EMPRESA_CODIGO AND A.PRESTA_CODIGO = B.PRESTA_CODIGO AND (A.EMPRESA_CODIGO = '" & Session("CodEmpresa") & "') " _
                                                      & " AND (B.PREDET_ESTADO_PRESTAMO = '1') AND (A.PRESTA_TIPO_MOVIMIENTO = 'S') AND (A.PRESTA_TIPOORIGEN = '1') AND (A.ALMACEN_CODIGO_ORIGEN =" & lblCodDestino.Text & ") AND (A.PRESTA_TIPODESTINO = '2') AND (A.CECOSE_CODIGO_DESTINO = " & lblCodOrigen.Text & ") AND (B.SERIE_NUMERAR = " & .Rows(i).Cells(6).Text & ")"
                                CmdGlobal2.ExecuteNonQuery()
                            Else 'ccosto
                                CmdGlobal2.CommandText = "UPDATE TBINV_PRESTAMO_DETALLE SET PREDET_ESTADO_PRESTAMO = '2',OSAL_CODIGO_DEVOL =" & lblCodigo.Text & ",DESP_CODIGO_DEVOL = NULL,RECEP_CODIGO_DEVOL = NULL FROM TBINV_PRESTAMO A, TBINV_PRESTAMO_DETALLE B WHERE A.EMPRESA_CODIGO = B.EMPRESA_CODIGO AND A.PRESTA_CODIGO = B.PRESTA_CODIGO AND (A.EMPRESA_CODIGO = '" & Session("CodEmpresa") & "') " _
                                                      & " AND (B.PREDET_ESTADO_PRESTAMO = '1') AND (A.PRESTA_TIPO_MOVIMIENTO = 'S') AND (A.PRESTA_TIPOORIGEN = '2') AND (A.CECOSE_CODIGO_ORIGEN =" & lblCodDestino.Text & ") AND (A.PRESTA_TIPODESTINO = '2') AND (A.CECOSE_CODIGO_DESTINO = " & lblCodOrigen.Text & ") AND (B.SERIE_NUMERAR = " & .Rows(i).Cells(6).Text & ")"
                                CmdGlobal2.ExecuteNonQuery()
                            End If
                        Next
                    End With
                ...
                Case 12 'DEVOLUCION REEMPLAZO X CAMBIO
                    With _DetalleEq
                        For i = 0 To .Rows.Count - 1
                            CmdGlobal.CommandText = "SELECT * FROM TBINV_REEMPLAZOS WHERE REEM_NRO = '" & .Rows(i).Cells(9).Text & "' AND REEM_SYS_EST='0' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                            Rs = CmdGlobal.ExecuteReader
                            If Rs.HasRows Then
                                While Rs.Read
                                    CmdGlobal2.CommandText = "UPDATE TBINV_REEMPLAZOS SET REEM_ESTADO_1='3' , REEM_SYS_MOD='" & ValorSys & "',NRO_SALIDA_CC='" & lblCodigo.Text & "' WHERE REEM_NRO='" & .Rows(i).Cells(9).Text & "' AND REEM_SYS_EST='0' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                                    CmdGlobal2.ExecuteNonQuery()
                                End While
                            End If
                            Rs.Close()
                ...
                Case 13 'DEVOLUCION X AVERIA
                    With _DetalleEq
                        For i = 0 To .Rows.Count - 1
                            Dim cAveria As DropDownList = .Rows(i).Cells(11).FindControl("cboAveria")
                            Dim tDetAveria As TextBox = .Rows(i).Cells(13).FindControl("txtDetAveria")
                            CmdGlobal2.CommandText = "SELECT * FROM TBINV_AVERIA WHERE AVERIA_NRO ='" & .Rows(i).Cells(10).Text & "' AND AVERIA_SYS_EST='0' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                            Rs = CmdGlobal.ExecuteReader
                            If Rs.HasRows Then
                                While Rs.Read
                                    If cAveria.SelectedValue <> "< Seleccionar >" Then
                                        CmdGlobal2.CommandText = " UPDATE TBINV_AVERIA SET AVERIA_ESTADO_1='1' ,AVERIA_FECHA ='" & Format(FechaSal, "yyyymmdd") & "',AVERIA_TIPO ='" & cAveria.SelectedValue & "', " _
                                                      & " AVERIA_DETALLE_USUARIO='" & tDetAveria.Text.Trim & "', AVERIA_SYS_MOD='" & ValorSys & "', SALIDA_NRO ='" & lblCodigo.Text & "' WHERE AVERIA_NRO ='" & .Rows(i).Cells(10).Text & "' AND AVERIA_SYS_EST='0' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                                        CmdGlobal2.ExecuteNonQuery()
                                    End If
                                End While
                            End If
                            Rs.Close()
                            CmdGlobal2.CommandText = "SELECT * FROM TBINV_REEMPLAZOS WHERE REEM_NRO = '" & .Rows(i).Cells(9).Text & "' AND REEM_SYS_EST='0' AND AVERIA_NRO ='" & .Rows(i).Cells(9).Text & "' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                            Rs = CmdGlobal.ExecuteReader
                            If Rs.HasRows Then
                                While Rs.Read
                                    CmdGlobal2.CommandText = " UPDATE TBINV_REEMPLAZOS SET REEM_ESTADO_1='3' , REEM_SYS_MOD='" & ValorSys & "',NRO_SALIDA_CC='" & lblCodigo.Text & "' WHERE REEM_NRO='" & .Rows(i).Cells(9).Text & "' AND REEM_SYS_EST='0' AND EMPRESA_CODIGO='" & Session("CodEmpresa") & "'"
                        ...
                        For i = 0 To .Rows.Count - 1
                            ...
                            If cAveria.SelectedValue <> "< Seleccionar >" Then
                                CmdGlobal2.CommandText = " INSERT INTO TBINV_AVERIA (EMPRESA_CODIGO, AVERIA_NRO, AVERIA_FECHA, AVERIA_TIPO, SALIDA_NRO, " _
                                                  ...
                                                  & " '" & tDetAveria.Text.Trim & "','" & .Rows(i).Cells(6).Text & "','1','1','2','" & lblCodOrigen.Text & "', " _
                                                  & " '" & IIf(RBAlmacen.Checked = True, "1", "2") & "','" & lblCodDestino.Text & "','" & ValorSys & "','0','1')"
                                CmdGlobal2.ExecuteNonQuery()
                            End If
                        Next
                    End With
    ...
    Protected Sub btnRedirectYes_Click(sender As Object, e As EventArgs) Handles btnRedirectYes.Click
        ...
        Session("PaginaViene") = "Inventario_CCosto_Salida_Generar.aspx"
                ...
                Case Else
                    If Session("Busqueda") = "Accesorio" Then
                        ...
                        Sql = Sql & " ),0) AS STOCK_ACTUAL,ART_CODEQUIVA, A.ART_SKU, " _
                            ...
                            & " inner join dbo.TBINV_ARTICULO_CLASIFICACION AC ON AC.CLAS_CODIGO = A.ART_CLASIFICACION " _
                            & " WHERE  D.UBICACT_TIPO ='2' AND (D.ALMACEN_CODIGO = " & lblCodOrigen.Text & ") AND (D.EMPRESA_CODIGO = '" & Session("CodEmpresa") & "') AND (D.SAA_SYS_EST = '0') AND (A.ART_SYS_EST = '0')  AND (A.ART_TIPO = 87) "
                        'al stock actual le restamos los articulos q se tienen q devolver y/o falta devolver
                    Else
    ...
    Private Sub GvBusEquipo_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GvBusEquipo.RowCommand
        ...
        If e.CommandName = "AgregarFila" Then
            ...
            With _DetalleEq
                ...
                '    arrSelec(3, i) = .Rows(i).Cells(4).Text.Trim 'SERIE_NRO
                '    arrSelec(4, i) = .Rows(i).Cells(5).Text.Trim 'PLACA_NRO
                '    arrSelec(5, i) = .Rows(i).Cells(6).Text.Trim 'SERIE_NUMERAR
                ...
                '    arrSelec(8, i) = .Rows(i).Cells(8).Text.Trim 'REEN_NUMERO
                '    arrSelec(9, i) = .Rows(i).Cells(9).Text.Trim 'AVERIA
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    arrSelec(6, i) = cFuncion.SelectedValue.Trim  'COD_FUNCION
                    arrSelec(7, i) = cFuncion.SelectedIndex.ToString  'combo funcion index
                    arrSelec(8, i) = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(.Rows(i).Cells(8).Text.Trim, "&#250;", "ú"), "&#243;", "ó"), "&#237;", "í"), "&#233;", "é"), "&#225;", "á"), "&quot;", """"), "&#191;", "¿"), "&#241;", "ñ"), "&lt;", "<"), "&gt;", ">"), "&#176;", "º"), "&#211;", "Ó"), "&#193;", "Á"), "&#201;", "É"), "&#205;", "Í"), "&#218;", "Ú"), "&nbsp;", ""), "&#242;", "ò"), "&#236;", "ì"), "&#200;", "È"), "&#209;", "Ñ"), "&#180;", "´"), "&#186;", "°"), "&amp;", "&") 'REEN_NUMERO
                    arrSelec(9, i) = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(.Rows(i).Cells(9).Text.Trim, "&#250;", "ú"), "&#243;", "ó"), "&#237;", "í"), "&#233;", "é"), "&#225;", "á"), "&quot;", """"), "&#191;", "¿"), "&#241;", "ñ"), "&lt;", "<"), "&gt;", ">"), "&#176;", "º"), "&#211;", "Ó"), "&#193;", "Á"), "&#201;", "É"), "&#205;", "Í"), "&#218;", "Ú"), "&nbsp;", ""), "&#242;", "ò"), "&#236;", "ì"), "&#200;", "È"), "&#209;", "Ñ"), "&#180;", "´"), "&#186;", "°"), "&amp;", "&") 'AVERIA
                    Dim cAveria As DropDownList = _DetalleEq.Rows(i).Cells(11).FindControl("cboAveria")
                    arrSelec(10, i) = cAveria.SelectedValue.Trim  'COD_FALLA
                    arrSelec(11, i) = cAveria.SelectedIndex.ToString   'combo averia index
                    Dim tDetAveria As TextBox = _DetalleEq.Rows(i).Cells(13).FindControl("txtDetAveria")
            ...
            arrSelec(1, f) = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(GvBusEquipo.Rows(Index).Cells(2).Text.Trim, "&#250;", "ú"), "&#243;", "ó"), "&#237;", "í"), "&#233;", "é"), "&#225;", "á"), "&quot;", """"), "&#191;", "¿"), "&#241;", "ñ"), "&lt;", "<"), "&gt;", ">"), "&#176;", "º"), "&#211;", "Ó"), "&#193;", "Á"), "&#201;", "É"), "&#205;", "Í"), "&#218;", "Ú"), "&nbsp;", ""), "&#242;", "ò"), "&#236;", "ì"), "&#200;", "È"), "&#209;", "Ñ"), "&#180;", "´"), "&#186;", "°"), "&amp;", "&")
            ...
            For ii = 0 To f
                ...
                If arrSelec(7, ii) Is Nothing Then
                    _dr(6) = String.Empty
                Else
                    _dr(6) = arrSelec(7, ii).Trim
                End If
                If arrSelec(8, ii) Is Nothing Then
                    _dr(7) = String.Empty
                Else
                    _dr(7) = arrSelec(8, ii).Trim
                End If
                _dt.Rows.Add(_dr)
            Next
            ...
            With _DetalleEq
                For i = 0 To .Rows.Count - 1
                    ...
                    tDetAveria.Text = arrSelec(13, i).Trim
                Next
            End With
    ...
    Private Sub GvBusAcc_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GvBusAcc.RowCommand
        ...
        If e.CommandName = "AgregarFila" Then
            ...
            With _DetalleAc
                ...
                For i = 0 To .Rows.Count - 1
                    f = f + 1
                    ReDim Preserve arrSelec(7, f)
                    arrSelec(0, i) = .Rows(i).Cells(1).Text.Trim 'ARTICULO_CODIGO
                    arrSelec(1, i) = .Rows(i).Cells(2).Text.Trim 'ART_DESCRIPCION
                    arrSelec(2, i) = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(.Rows(i).Cells(3).Text.Trim(), "&#250;", "ú"), "&#243;", "ó"), "&#237;", "í"), "&#233;", "é"), "&#225;", "á"), "&quot;", """"), "&#191;", "¿"), "&#241;", "ñ"), "&lt;", "<"), "&gt;", ">"), "&#176;", "º"), "&#211;", "Ó"), "&#193;", "Á"), "&#201;", "É"), "&#205;", "Í"), "&#218;", "Ú"), "&nbsp;", ""), "&#242;", "ò"), "&#236;", "ì"), "&#200;", "È"), "&#209;", "Ñ"), "&#180;", "´"), "&#186;", "°"), "&amp;", "&")
                    'ART_sku
            ...
            arrSelec(2, f) = Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(GvBusAcc.Rows(index).Cells(4).Text.Trim(), "&#250;", "ú"), "&#243;", "ó"), "&#237;", "í"), "&#233;", "é"), "&#225;", "á"), "&quot;", """"), "&#191;", "¿"), "&#241;", "ñ"), "&lt;", "<"), "&gt;", ">"), "&#176;", "º"), "&#211;", "Ó"), "&#193;", "Á"), "&#201;", "É"), "&#205;", "Í"), "&#218;", "Ú"), "&nbsp;", ""), "&#242;", "ò"), "&#236;", "ì"), "&#200;", "È"), "&#209;", "Ñ"), "&#180;", "´"), "&#186;", "°"), "&amp;", "&")
            ...
            With _DetalleAc
                For i = 0 To .Rows.Count - 1
                    Dim tCantSal As TextBox = .Rows(i).Cells(4).FindControl("txtCantSal")
