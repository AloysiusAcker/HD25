...
#End Region
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            'If User.Identity.Name = "" Then   'Or Session("TipoGrupo") Is Nothing
            '    Session.Clear()
            ...
            Try
                ...
                cmdSql.CommandText = "SELECT R.PRUEBA_CODIGO, P.PREG_ORDEN, R.PREG_CODIGO, P.PREG_DESCRIPCION, R.RESP_ORDEN, R.RESP_CODIGO, R.RESP_DESCRIPCION " _
                   & " FROM TBGENERAC_PRUEBA_RESPUESTAS_0001 R INNER JOIN TBGENERAC_PRUEBA_PREGUNTAS_0001 P ON R.PRUEBA_CODIGO = P.PRUEBA_CODIGO AND R.PREG_CODIGO = P.PREG_CODIGO " _
                ...
                If Rs.HasRows Then
                    While Rs.Read
                        ...
                        '          & " WHERE (PRUEBA_CODIGO = " & Session("CodPrueba") & ") AND (GRUPO_TIPO = '6') AND (GRUPO_CODIGO = " & Session("CodGrupo") & ") AND (PREG_CODIGO = " & Nz(Rs!PREG_CODIGO) & ") AND (RESP_CODIGO = " & Nz(Rs!RESP_CODIGO) & ") AND (DD_RESPONDIDO = 'X') AND (DD_SYS_EST='0')"
                        cmdSql2.CommandText = "SELECT COUNT(DD_RESPONDIDO) FROM TBGENERAC_PRUEBA_DESA_DETALLE_0001 " _
                           & " WHERE (PRUEBA_CODIGO = " & Session("CodPrueba") & ") AND (GRUPO_TIPO = '6') AND (GRUPO_CODIGO = " & Session("CodGrupo") & ") AND (PREG_CODIGO = " & Nz(Rs!PREG_CODIGO) & ") AND (RESP_CODIGO = " & Nz(Rs!RESP_CODIGO) & ") AND (DD_RESPONDIDO = 'X') AND (DD_SYS_EST='0')"
                ...
                End If
                Rs.Close()
                cmdSql.CommandText = "SELECT PREG_ORDEN, PREG_DESCRIPCION,PRUEBA_CODIGO,PREG_CODIGO FROM TBGENERAC_PRUEBA_PREGUNTAS_0001 P WHERE (PREG_SYS_EST = '0') AND (PRUEBA_CODIGO =" & Session("CodPrueba") & ") ORDER BY PREG_ORDEN"
                Rs = cmdSql.ExecuteReader
                If Rs.HasRows Then
                    While Rs.Read
                        a = 0
                        cmdSql2.CommandText = "SELECT DD.PREG_CODIGO, MAX(DD.DD_RESPONDIDO) AS VALOR, D." & TipoCampoGrupo & ", D.PD_ESTADO " _
                             & "FROM TBGENERAC_PRUEBA_DESA_DETALLE_0001 DD INNER JOIN TBGENERAC_PRUEBA_DESARROLLO_0001 D ON DD.PRUEBA_CODIGO = D.PRUEBA_CODIGO AND " _
                             & "Dd.GRUPO_CODIGO = d.GRUPO_CODIGO And Dd.GRUPO_TIPO = d.GRUPO_TIPO And Dd." & TipoCampoGrupo & " = d." & TipoCampoGrupo & " " _
                             & "WHERE (DD.PRUEBA_CODIGO = " & Session("CodPrueba") & ") AND (DD.GRUPO_TIPO = '6') AND (DD.GRUPO_CODIGO =" & Session("CodGrupo") & ") AND (D.PD_SYS_EST = '0') AND (DD.DD_SYS_EST = '0') " _
                        ...
                        'cmdSql2.CommandText = "SET NOCOUNT ON DECLARE @TT TABLE(PD_NUM_ANONIMO FLOAT) INSERT INTO @TT SELECT D." & TipoCampoGrupo & " FROM TBGENERAC_PRUEBA_DESA_DETALLE_" & Session("CodEmpresa") & " DD INNER JOIN TBGENERAC_PRUEBA_DESARROLLO_" & Session("CodEmpresa") & " D ON DD.PRUEBA_CODIGO = D.PRUEBA_CODIGO AND " _
                        '     & "Dd.GRUPO_CODIGO = d.GRUPO_CODIGO And Dd.GRUPO_TIPO = d.GRUPO_TIPO And Dd." & TipoCampoGrupo & " = d." & TipoCampoGrupo & " " _
                        '     & "WHERE (DD.PRUEBA_CODIGO = " & Session("CodPrueba") & ") AND (DD.GRUPO_TIPO = '6') AND (DD.GRUPO_CODIGO =" & Session("CodGrupo") & ") AND (D.PD_SYS_EST = '0') AND (DD.DD_SYS_EST = '0') " _
                        '     & "GROUP BY DD.PREG_CODIGO, D." & TipoCampoGrupo & ",D.PD_ESTADO " _
                        '     & "HAVING (DD.PREG_CODIGO =" & Nz(Rs!PREG_CODIGO) & ") AND (D.PD_ESTADO = '3') AND (MAX(DD.DD_RESPONDIDO) IS NULL OR MAX(DD.DD_RESPONDIDO) = '') SELECT @@ROWCOUNT"
                        Rs2 = cmdSql2.ExecuteReader
                        If Rs2.HasRows Then
                            While Rs2.Read
                                a = a + 1
                ...
                Rs.Close()
                a = 0
                cmdSql.CommandText = "SELECT * FROM TBGENERAC_PRUEBA_DESARROLLO_0001 WHERE (PRUEBA_CODIGO =" & Session("CodPrueba") & ") AND (GRUPO_CODIGO =" & Session("CodGrupo") & ") AND (GRUPO_TIPO = '6') AND (PD_SYS_EST = '0')  AND (PD_ESTADO = '3')"
                Rs = cmdSql.ExecuteReader
                If Rs.HasRows Then
                    While Rs.Read
                        a = a + 1
                ...
                If Rs.HasRows Then
                    ...
                    While Rs.Read
                        ...
                        lblTotalEnc.Text = "Personas que desarrollaron la Encuesta : " & Nz(Rs!TOTAL_DESARROLLADOS)
                        b = b + 1
                        dr = dt.NewRow()
                        dr(0) = "<b>" & b.ToString & ".- "
